...
class IndexLookup(Layer):
    ...
    def _validate_idf_weights(self, idf_weights):
        """Validate if `idf_weights` is valid according to the output_mode."""
        if self.output_mode == "tf_idf":
            ...
            raise ValueError(
                ...
                f"Received: output_mode={self.output_mode} and idf_weights={idf_weights}"
            )

    def _validate_vocabulary(self, vocabulary):
        """Ensure the vocabulary is not empty and is a valid format."""
        if vocabulary.size == 0:
            raise ValueError(f"Cannot set an empty vocabulary. Received: vocabulary={vocabulary}")

    def _handle_tensor_vocabulary(self, vocabulary, idf_weights):
        """Handle conversion of tensor inputs for vocabulary and idf_weights."""
    ...
    def _check_special_tokens(self, vocabulary):
        """Check if the special tokens are correctly placed in the vocabulary."""
        ...
        return tokens, found_special_tokens

    def _check_for_repeated_tokens(self, tokens):
        """Check if there are any repeated tokens in the vocabulary."""
        repeated_tokens = self._find_repeated_tokens(tokens)
        if repeated_tokens:
            raise ValueError(
                "The passed vocabulary has at least one repeated term. "
                "Please uniquify your dataset. The repeated terms are: "
                f"{repeated_tokens}"
            )

    def _check_for_reserved_tokens(self, tokens, vocabulary, found_special_tokens):
        """Check for any reserved tokens (mask_token or oov_token) in invalid positions."""
        if self.mask_token is not None and self.mask_token in tokens:
            ...
            raise ValueError(
                ...
                "`vocabulary`. Either remove all mask and OOV tokens, or include them "
                "only at the start of the vocabulary in precisely this order."
                f"Received: mask_token={self.mask_token} at vocabulary index {mask_index}"
            )
        ...
        if self.oov_token is not None and self.invert and self.oov_token in tokens:
            ...
            raise ValueError(
                ...
                "`vocabulary`. Either remove all mask and OOV tokens, or include them "
                "only at the start of the vocabulary in precisely this order."
                f"Received: oov_token={self.oov_token} at vocabulary index {oov_index}"
            )

    def _pad_idf_weights(self, idf_weights, found_special_tokens, token_start):
        """Pad the IDF weights if necessary."""
        front_padding = 0 if found_special_tokens else token_start
        front_padding_value = 0 if found_special_tokens else np.average(idf_weights)
        ...
        if self.pad_to_max_tokens and self.max_tokens is not None:
            ...
            back_padding = 0
        return np.pad(
            idf_weights, (front_padding, back_padding), "constant", constant_values=(front_padding_value, back_padding_value)
        )
    ...
    def set_vocabulary(self, vocabulary, idf_weights=None):
        ...
        if not tf.executing_eagerly() and (tf.is_tensor(vocabulary) or tf.is_tensor(idf_weights)):
            raise RuntimeError(
                f"Cannot set a tensor vocabulary on layer {self.name} when not executing eagerly."
                " Create this layer or call `set_vocabulary()` outside of any traced function."
            )

        vocabulary, idf_weights = self._handle_tensor_vocabulary(vocabulary, idf_weights)
        self._validate_vocabulary(vocabulary)

        tokens, found_special_tokens = self._check_special_tokens(vocabulary)
        self._check_for_repeated_tokens(tokens)
        self._check_for_reserved_tokens(tokens, vocabulary, found_special_tokens)
        ...
        if self.max_tokens is not None and new_vocab_size > self.max_tokens:
            raise ValueError(
                ...
                f"Received vocabulary size is {new_vocab_size}; `max_tokens` is {self.max_tokens}."
            )
        ...
        if self.output_mode == "tf_idf" and idf_weights is not None:
            idf_weights = self._pad_idf_weights(idf_weights, found_special_tokens, self._token_start_index())
            weights = tf.convert_to_tensor(idf_weights, dtype=backend.floatx())
